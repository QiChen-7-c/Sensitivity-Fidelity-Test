...
from libs.tools import LpLoss, count_params
from train_AFNO import (
    apply_config_overrides,
    broadcast_run_directory,
    cleanup_distributed,
    distributed_max,
...
def parse_args() -> Tuple[argparse.Namespace, List[str]]:
    parser = argparse.ArgumentParser(
        description="Distributed training entry for Swin neural operator (RB2D dataset)"
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=Path(__file__).resolve().parent / "configs/swin.yaml",
...
def main():
    ...
    if main_process:
        try:
            with args.config.open("r") as handle:
                config_for_checkpoint = yaml.safe_load(handle)
            if overrides:
    ...
    if main_process:
        run_dir = create_run_directory(config, "swin", args.seed)
    else:
        run_dir = Path(".")
    ...
    model = build_model(config, "swin").to(device)
