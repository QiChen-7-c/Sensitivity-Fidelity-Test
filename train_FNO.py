...
def parse_args() -> Tuple[argparse.Namespace, List[str]]:
    parser = argparse.ArgumentParser(
        description="Distributed training entry for FNO (RB2D dataset)"
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=Path(__file__).resolve().parent / "configs/fno.yaml",
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Process at most this many training batches per epoch.",
    )
    parser.add_argument(
        "--max-val-batches",
        type=int,
        default=None,
        help="Process at most this many validation batches per epoch.",
    )
    parser.add_argument(
        "--epoch-one-step",
        type=int,
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Enable activation checkpointing for the FNO blocks.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--activation-checkpoint-use-reentrant",
        dest="activation_checkpoint_use_reentrant",
        action="store_true",
        default=False,
        help="Use the reentrant checkpoint backend. Default is False when supported by PyTorch.",
    )
    parser.add_argument(
        "--no-activation-checkpoint-use-reentrant",
        dest="activation_checkpoint_use_reentrant",
        action="store_false",
        help="Disable reentrant checkpointing.",
    )
...
def init_distributed_mode(args: argparse.Namespace):
    if not dist.is_available():
        raise RuntimeError("torch.distributed is required for train_FNO.py")
...
def distributed_barrier(
    ...
    *,
    ...
    try:
        ...
        if suppress_errors:
            warnings.warn(f"Ignoring distributed {context} error during shutdown: {exc}")
            return False
        raise


def release_cuda_memory(device: Optional[torch.device]):
    gc.collect()
    ...
    *,
    ...
    if dist.is_available() and dist.is_initialized():
        ...
        try:
            ...
            if suppress_errors:
                warnings.warn(
                    "Ignoring distributed shutdown error after training artifacts were "
                    f"already written: {exc}"
                )
            else:
                raise
...
def model_has_complex_state(module: torch.nn.Module) -> bool:
...
def main():
    ...
    if config.model.in_time_window != 1:
        raise ValueError(
            "train_FNO.py currently supports in_time_window=1. "
            "Please set model.in_time_window to 1 in the config."
        )
    ...
    args_snapshot.update(
        {
            ...
            "activation_checkpoint_use_reentrant": bool(args.activation_checkpoint_use_reentrant),
    ...
    train_loader, val_loader, train_sampler, val_sampler = prepare_dataloaders(
        ...
        # train_subset_ratio=0.001, # for quick debugging
    )

    if main_process:
        run_dir = create_run_directory(config, "fno", args.seed)
    else:
        run_dir = Path(".")
    run_dir = broadcast_run_directory(run_dir)
    distributed_barrier(device, local_rank)

    log_file = run_dir / "training_epoch_log.txt"
    model = build_model(
        config, "fno",
        activation_checkpointing=args.activation_checkpointing,
        activation_checkpoint_use_reentrant=args.activation_checkpoint_use_reentrant,
    ).to(device)
    has_complex_state = model_has_complex_state(model)
    param_info = (
        f"count_params: {count_params(model)} | "
        f"activation_checkpointing: {bool(args.activation_checkpointing)} | "
        f"use_reentrant: {bool(args.activation_checkpoint_use_reentrant)}"
    )
    ...
    if scaler is not None and has_complex_state:
        scaler = None
        if main_process:
            warnings.warn(
                "Disabling AMP GradScaler because model parameters include complex tensors. "
                "torch.amp GradScaler does not support complex gradient unscale."
            )
    ...
    try:
        for epoch in range(total_epochs):
            ...
            if device.type == "cuda":
                torch.cuda.synchronize(device=device)
                peak_mem_bytes = float(torch.cuda.max_memory_allocated(device=device))
                mem_usage = distributed_max(peak_mem_bytes, device) / 1024**3
            else:
                mem_usage = 0.0
        ...
        training_artifacts_written = True

        # Release model and optimizer state before the final NCCL sync/destroy
        # so shutdown still has room for communicator cleanup allocations.
        del model, optimizer, scheduler, scaler, loss_fn
        release_cuda_memory(device)
        final_barrier_ok = distributed_barrier(
            device,
            local_rank,
            suppress_errors=True,
            context="final barrier",
        )
        ...
        if main_process:
            ...
            if not final_barrier_ok:
                print(
                    "Final distributed shutdown reported a CUDA/NCCL warning after "
                    "checkpoints were already saved."
                )
            print("=" * 50)
    finally:
        cleanup_distributed(
            device=device,
            suppress_errors=training_artifacts_written,
        )
