...
def parse_args() -> Tuple[argparse.Namespace, List[str]]:
    parser = argparse.ArgumentParser(
        description="Distributed training entry for AFNO (RB2D dataset)"
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=Path(__file__).resolve().parent / "configs/afno.yaml",
...
def init_distributed_mode(args: argparse.Namespace):
    if not dist.is_available():
        raise RuntimeError("torch.distributed is required for train_AFNO.py")
...
def main():
    ...
    if main_process:
        run_dir = create_run_directory(config, "afno", args.seed)
    else:
        run_dir = Path(".")
    ...
    model = build_model(config, "afno").to(device)
    activation_checkpointing = bool(getattr(config.model, "activation_checkpointing", False))
    param_info = (
    ...
    try:
        for epoch in range(total_epochs):
            ...
            if device.type == "cuda":
                ...
                memory_snapshot = get_cuda_memory_snapshot(device)
                peak_allocated_gb = distributed_max(
                    float(memory_snapshot["peak_allocated"]),
                    device,
                ) / 1024**3
                peak_reserved_gb = distributed_max(
                    float(memory_snapshot["peak_reserved"]),
                    device,
                ) / 1024**3
            else:
